...
! program eigenvalue_analysis
!     use, intrinsic :: iso_c_binding
!     implicit none
  
!     ! 定数定義
!     integer, parameter :: NSP = 10  ! NSP の値は適切に設定すること
!     real(8), parameter :: t = 0.0d0
!     real(8), parameter :: pres = 101325.0d0
  
!     ! 変数定義
!     real(8), target :: y(NSP) = (/1000.d0, 0.d0, 0.04030952d0, 0.d0, 0.95969048d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0/)
!     real(8), target :: jac(NSP, NSP)
!     real(8) :: wr(NSP), wi(NSP)   ! 固有値の実部と虚部
!     real(8) :: vl(NSP, NSP)       ! 左固有ベクトル
!     real(8) :: vr(NSP, NSP)       ! 右固有ベクトル
!     real(8) :: work(4 * NSP)      ! 作業配列
!     integer :: lwork, info
!     real(8) :: a_exp(NSP), b_exp(NSP)
!     real(8) :: EP(NSP), EI(NSP)
...
!   end program eigenvalue_analysis
